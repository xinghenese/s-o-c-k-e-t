...
public class Snappy
{
     ...
     */
    public static int calculateChecksum(InputByteBuffer data)
    {
     ...
     *            The input data to calculate the CRC32C checksum of
     */
    public static int calculateChecksum(InputByteBuffer data, int offset, int length)
    {
    ...
    public void decode(InputByteBuffer in, OutputByteBuffer out)
    {
        while (in.isReadable())
        {
            switch (state)
    ...
    public void encode(InputByteBuffer in, OutputByteBuffer out)
    {
        ...
        if (length - inIndex >= MIN_COMPRESSIBLE_BYTES)
        {
            ...
            outer: while (true)
            {
                ...
                do
                {
                    ...
                    in.setIndex(in.getIndex() + matched);
     ...
     */
    static void validateChecksum(int expectedChecksum, InputByteBuffer data, int offset, int length)
    {
     ...
     *             If the calculated and supplied checksums do not match
     */
    static void validateChecksum(int expectedChecksum, InputByteBuffer data)
    {
     ...
     */
    private static int decodeCopyWith1ByteOffset(byte tag, InputByteBuffer in, OutputByteBuffer out, int writtenSoFar)
    {
        if (!in.isReadable())
        {
        ...
        if (offset < length)
        {
            ...
            for (; copies > 0; copies--)
            {
                out.writeBytes(out.array(), initialIndex - offset, offset);
            }
            if (length % offset != 0)
            {
                out.writeBytes(out.array(), initialIndex - offset, length % offset);
            }
        }
        else
        {
            out.writeBytes(out.array(), initialIndex - offset, length);
        }
     ...
     *             If the read offset is invalid
     */
    private static int decodeCopyWith2ByteOffset(byte tag, InputByteBuffer in, OutputByteBuffer out, int writtenSoFar)
    {
        ...
        int offset = Bytes.swapShort(in.readShort());
     ...
     *             If the read offset is invalid
     */
    private static int decodeCopyWith4ByteOffset(byte tag, InputByteBuffer in, OutputByteBuffer out, int writtenSoFar)
    {
        ...
        int initialIndex = out.getIndex();
        int length = 1 + (tag >> 2 & 0x03F);
        int offset = Bytes.swapInt(in.readInt());

        validateOffset(offset, writtenSoFar);

        InputByteBuffer inBuf = new InputByteBuffer(out.getBytes());
        // out.markReaderIndex();
        if (offset < length)
        {
            int copies = length / offset;
            for (; copies > 0; copies--)
            {
                out.writeBytes(inBuf.array(), initialIndex - offset, offset);
            }
            if (length % offset != 0)
            {
                out.writeBytes(inBuf.array(), initialIndex - offset, length % offset);
            }
        }
        else
        {
            out.writeBytes(inBuf.array(), initialIndex - offset, length);
        }
     ...
     *         indicate "try again later"
     */
    private static int decodeLiteral(byte tag, InputByteBuffer in, OutputByteBuffer out)
    {
        in.markIndex();
        int length;
        switch (tag >> 2 & 0x3F)
        {
            ...
            case 61:
                ...
                length = Bytes.swapShort(in.readShort());
                break;
            case 62:
                ...
                length = Bytes.swapMedium(in.readUnsignedMedium());
                break;
            case 64:
                if (in.readableBytes() < 4)
                {
                ...
                length = Bytes.swapInt(in.readInt());
                break;
            default:
                length = tag >> 2 & 0x3F;
        }
        ...
        out.writeBytes(in.array(), in.getIndex(), length);
        in.setIndex(in.getIndex() + length);
        return length;
    }
     ...
     */
    private static void encodeCopy(OutputByteBuffer out, int offset, int length)
    {
        while (length >= 68)
        {
    ...
    private static void encodeCopyWithOffset(OutputByteBuffer out, int offset, int length)
    {
     ...
     */
    private static void encodeLiteral(InputByteBuffer in, OutputByteBuffer out, int length)
    {
        if (length < 61)
        {
     ...
     */
    private static int findMatchingLength(InputByteBuffer in, int minIndex, int inIndex, int maxIndex)
    {
        int matched = 0;
     ...
     */
    private static int hash(InputByteBuffer in, int index, int shift)
    {
     ...
     */
    private static int readPreamble(InputByteBuffer in)
    {
        int length = 0;
        int byteIndex = 0;
        while (in.isReadable())
