...
package net.gimite.websocket 
{
	...
	public class WebSocket extends EventDispatcher
	{
		...
		public function WebSocket(id:int, url:String, protocols:Array, origin:String,proxyHost:String, proxyPort:int,cookie:String, headers:String,logger:IWebSocketLogger)	
		{
			...
			if (!m)
			{
				fatal("SYNTAX_ERR: invalid url: " + url);
			}
			this.scheme = m[1];
			this.host = m[2];
		...
		*/
		...
		public function close(code:int = STATUS_NO_CODE, reason:String = "", origin:String = "client"):void 
		{
		...
		private function onSocketIoError(event:IOErrorEvent):void 
		{
			...
			{
				message =
				"error communicating with Web Socket server at " + url + " (IoError: " + event.text + ")";
			}
			...
			if (headerState == 4) 
				{
					...
					if (!validateHandshake(headerStr))
					{
						return;
					}
